...
2. **去除IRFetch/ID周期中暂存投机计算结果的寄存器Target的写使能`BrWr`；`ALU`计算结果回写的数据源（`ALUOut`）更改为来自`Target`寄存器之后，而不是直接来自`ALU`输出端。**
   ...
   - 对于后者：这带来的好处是，进行R-Type和I指令运算后，第四周期不需要保持第三周期中的所有控制信号了，第三周期也无需置`RegDst`，因为运算结果也会保存在`Target`中，直接取出即可。能够减少状态数。（此点将在[控制单元](####2.2.3 控制单元)中详细描述）。
...
整体结构如下，黑色部分为[数据通路](####2.2.2 数据通路)，蓝色部分为[控制单元](####2.2.3 控制单元)。详细设计将在后续介绍。下图中，红色部分为相较于「`./references/slide.pptx`」的更改内容，解释如下：
